...
export function CandidateDetail() {
  const { id } = useParams({ from: "/_app/candidates/$id" });
  ...
  if (!c) {
    return (
      <div className="mx-auto max-w-3xl py-20 text-center">
        ...
        <Button asChild className="mt-6 rounded-xl"><Link to="/candidates">Back to candidates</Link></Button>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <Button asChild variant="ghost" size="sm" className="rounded-lg -ml-2">
        <Link to="/candidates"><ArrowLeft className="mr-1 h-4 w-4" /> Back to candidates</Link>
      </Button>
