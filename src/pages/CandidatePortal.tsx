...
import { assignedInterviews, completedResults } from "@/data/candidatePortal";
      ...
      <div className="flex min-h-screen w-full bg-background">
        ...
        <div className="flex min-w-0 flex-1 flex-col">
          ...
          <main className="flex-1 px-6 py-8">
            {view === "dashboard" ? <DashboardView /> : <ResultsView />}
          </main>
        </div>
      </div>
    </SidebarProvider>
  ...
  return (
    <Sidebar collapsible="icon" className="border-r">
      <SidebarHeader className="border-b">
        <Link to="/candidate" className="flex items-center gap-2.5 px-2 py-2.5">
...
function DashboardView() {
  const pending = assignedInterviews.filter((i) => i.status !== "completed");
  const completed = assignedInterviews.filter((i) => i.status === "completed");
  ...
  return (
    <div className="space-y-8">
      <section className="relative overflow-hidden rounded-3xl border bg-gradient-to-br from-primary/10 via-background to-ai/10 p-8">
        ...
        <h1 className="mt-2 text-3xl font-semibold tracking-tight">Hi Mira — ready for your next interview?</h1>
      ...
      <section>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {pending.map((iv) => (
            <Card key={iv.id} className="rounded-2xl border-border/60 shadow-soft transition hover:shadow-elegant">
              <CardHeader className="flex flex-row items-start justify-between space-y-0 pb-3">
                <div>
                  <Badge variant="secondary" className="rounded-full text-[10px]">{iv.category}</Badge>
                  <CardTitle className="mt-2 text-base">{iv.title}</CardTitle>
                  <p className="text-xs text-muted-foreground">{iv.company}</p>
                </div>
              ...
              <CardContent className="space-y-4">
                <div className="flex flex-wrap gap-3 text-xs text-muted-foreground">
                  <span className="flex items-center gap-1.5"><Clock className="h-3.5 w-3.5" /> {iv.duration}</span>
                  <span className="flex items-center gap-1.5"><Sparkles className="h-3.5 w-3.5" /> {iv.questions} questions</span>
                  <span className="flex items-center gap-1.5"><Calendar className="h-3.5 w-3.5" /> Due {iv.dueDate}</span>
                </div>
                <Button asChild className="w-full rounded-xl bg-gradient-primary text-white shadow-elegant">
                  <Link to="/interview/$id" params={{ id: iv.id }}>
              ...
              {completedResults.map((r) => (
                <li key={r.id} className="flex items-center justify-between gap-4 p-4">
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs text-muted-foreground">{r.date} · {r.duration} · {r.answers.length} answers</p>
                  </div>
                  <div className="text-right">
                    <p className="text-2xl font-semibold text-gradient">{r.score}</p>
                    <p className="text-[10px] uppercase tracking-wider text-muted-foreground">score</p>
                  </div>
                  <Button asChild variant="outline" size="sm" className="rounded-lg">
                    <Link to="/candidate/result/$id" params={{ id: r.id }}>
