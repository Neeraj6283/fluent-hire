...
export function Interviews() {
  ...
  const fetchInterviews = async () => {
    ...
    try {
      ...
      toast.error("An error occurred while loading interviews");
    } finally {
      setLoading(false);
    }
  };
  ...
  const confirmDelete = async () => {
    ...
    try {
      const response = await fetch(`/api/interviews/${pendingDelete}`, {
        method: "DELETE",
      });
      if (response.ok) {
                    ...
                    <tr key={r.id} className="border-t transition hover:bg-muted/40">
                      ...
                      <td className="px-3 py-3">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <Link href={`/interviews/${r.id}`} className="font-medium hover:underline">
                              {r.title}
                            </Link>
                            <p className="text-xs text-muted-foreground">~ {r.duration}m</p>
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="px-3 py-3">
                        {r.status === "Public" || r.status === "Published" ? (
