...
export function CreateInterview() {
  ...
  const generate = async () => {
    ...
    try {
      ...
      toast.success("AI has generated your interview questions!");
    } catch (error: any) {
  ...
  const handleFinish = async () => {
    ...
    try {
      ...
      const data = await response.json();
      if (!response.ok) throw new Error(data.error || "Failed to save interview");

      toast.success(`Interview saved as ${isPublic ? "Public" : "Draft"}!`);
    ...
    <div className="space-y-8">
        ...
        <Card className="rounded-2xl shadow-soft">
          <CardContent className="space-y-7 p-6 md:p-8">
            ...
            <div className="flex items-center justify-end gap-3 pt-2">
              <Button variant="outline" className="h-10 rounded-xl px-6" asChild>
                <Link href="/dashboard">Cancel</Link>
              </Button>
              <Button
                className="h-10 rounded-xl bg-gradient-primary px-8 text-white shadow-elegant"
      ...
      {step === 2 && (
        <div className="space-y-4">
                      ...
                      <div key={q.id} className="group flex gap-3 rounded-xl border p-4 transition hover:border-primary/40 animate-fade-up">
                        ...
                        <div className="flex-1">
                          ...
                          <Textarea
                            ...
                            rows={2}
                            onChange={(e) => {
                              setQuestions(prev => prev.map(item => item.id === q.id ? { ...item, text: e.target.value } : item));
                            }}
          ...
          <div className="flex items-center justify-between rounded-2xl border bg-card p-4 shadow-soft">
            <Button variant="outline" className="h-10 rounded-xl px-6" onClick={() => setStep(1)}>
            ...
            >
              Review interview <ArrowRight className="ml-2 h-4 w-4" />
            </Button>
          </div>
        </div>
      )}
      ...
      {step === 3 && (
        <div className="space-y-6">
            ...
            <div className="space-y-4">
              <Card className="rounded-2xl shadow-soft">
                <CardContent className="p-6">
                  <h4 className="font-semibold text-sm mb-4">Summary</h4>
                  <div className="space-y-3">
                    ...
                    <div className="flex items-center justify-between rounded-lg border bg-muted/30 px-3 py-2 text-sm">
                      ...
                      <span className="font-medium flex items-center gap-1.5">
                        <Clock className="h-3.5 w-3.5" /> {duration[0]} Min
                      </span>
                    </div>
                  </div>
                </CardContent>
          ...
          <div className="flex items-center justify-between rounded-2xl border bg-card p-4 shadow-soft">
            <Button variant="outline" className="h-10 rounded-xl px-6" onClick={() => setStep(2)}>
